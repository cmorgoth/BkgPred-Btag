...
int RatioPlots(TH1F* h1, TH1F* h2, TString h1Name = "h1Name", TString h2Name = "h2Name", TString fname = "default_name", TString type = "defaultType"){
  ...
  double R2Bins = 4;
  double c1B_tt[] = {0.50, 0.625, 0.80, 1.05, 1.2};
  ...
  if(type == "MR"){
    ...
    RATIO = new TH1F("RATIO", fname + "_" + type , R2Bins, c1B_tt);
    label = "R^{2}";
  ...
  leg = new TLegend(0.68, 0.77, 0.89, 0.9);//(xmin, ymin, xmax, ymax)
  leg->AddEntry(h1, label + " " + h1Name ,"lep");
  leg->AddEntry(h2, label + " " + h2Name ,"lep");
  leg->SetTextSize(.035);
  leg->SetFillColor(0);
  leg->SetLineColor(0);
  leg->Draw();
...
int BandMC_TGraph(TGraphAsymmErrors* h1, TH1F* h2, TString h1Name = "h1Name", TString h2Name = "h2Name", TString fname = "default_name", TString type = "defaultType", int nbins = 0, float* bins = NULL, int color = 0){
  ...
  h1->GetXaxis()->SetRangeUser(0.5, 1.2);
  h1->GetYaxis()->SetRangeUser(0.01*h2->GetBinContent(h2->GetNbinsX()), 1.5*h2->GetBinContent(1));
  h2->SetXTitle( type );
  ...
  leg = new TLegend(0.6, 0.7, 0.85, 0.85);//(xmin, ymin, xmax, ymax)
